...
## ⚠️ Note on Tool Selection (OpenAI → Free Alternatives)
...
├── rag_app.py           # Core RAG logic, sample documents, and Q&A demo
├── document_loader.py   # Utilities to load PDFs, URLs, and text files
...
> ⚠️ The first run will download the HuggingFace embedding model (~80MB). This is automatic and only happens once.
